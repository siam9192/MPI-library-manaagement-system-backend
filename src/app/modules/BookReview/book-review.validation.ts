...
const createBookReview = z.object({
 borrowId:z.string({required_error:"borrowId is required"}).refine(val=>isValidObjectId(val),{message:"Invalid borrowId"}),
...
const changeBookReviewStatus  = z.object( {
 status:z.enum([EBookReviewStatus.VISIBLE,EBookReviewStatus.HIDDEN],{message:"Invalid status"})
} )
