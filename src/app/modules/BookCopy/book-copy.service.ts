...
import {
  EBookCopyStatus,
  ICreateBookCopyPayload,
  IUpdateBookCopyPayload,
} from './book-copy.interface';
...
class BookCopyService {
  async createBookCopyIntoDB(payload: ICreateBookCopyPayload) {
    ...
    try {
      ...
      throw new AppError(
        ...
        'Internal server error!.Book copy could not be created'
      );
    } finally {
      session.endSession();
    }
  ...
  async deleteBookCopyIntoDB(id: string) {
    ...
    if (
      [EBookCopyStatus.CHECKED_OUT, EBookCopyStatus.LOST, EBookCopyStatus.RESERVED].includes(
        copy.status
      )
    ) {
      throw new AppError(
